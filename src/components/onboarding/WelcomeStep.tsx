...
const WelcomeStep: React.FC<WelcomeStepProps> = ({ onStart }) => {
  return (
    <div className="min-h-dvh flex flex-col items-center justify-center px-6 relative overflow-hidden">
      {/* Aurora Background - Viola/Indigo gradient */}
      ...
      <div className="relative z-10 flex flex-col items-center text-center">
        {/* Animated Avatar with Aurora gradient */}
        <motion.div 
        ...
        >
          ...
          <div className="absolute inset-0 w-36 h-36 rounded-full bg-gradient-aria blur-xl opacity-40 animate-pulse" />
          
          {/* Main avatar circle with Aurora gradient border */}
          <div className="relative w-36 h-36 rounded-full bg-gradient-to-br from-aria-violet/20 to-aria-indigo/10 flex items-center justify-center border border-aria-violet/30 shadow-aria-glow backdrop-blur-xl">
            <motion.div
              animate={{ scale: [1, 1.05, 1] }}
              transition={{ repeat: Infinity, duration: 3, ease: "easeInOut" }}
            >
              <Sparkles className="w-16 h-16 text-aria-violet" />
            </motion.div>
          </div>
          
          {/* Floating particles - Aurora colors */}
          <motion.div 
            className="absolute -top-3 -right-3 w-4 h-4 bg-aria-violet/60 rounded-full"
            animate={{ y: [-3, 3, -3], x: [-2, 2, -2] }}
            transition={{ repeat: Infinity, duration: 2.5, ease: "easeInOut" }}
          />
          <motion.div 
            className="absolute -bottom-2 -left-4 w-3 h-3 bg-aria-indigo/50 rounded-full"
            animate={{ y: [3, -3, 3], x: [2, -2, 2] }}
            transition={{ repeat: Infinity, duration: 3, ease: "easeInOut" }}
          />
          <motion.div 
            className="absolute top-1/2 -right-5 w-2 h-2 bg-aria-purple/50 rounded-full"
            animate={{ y: [-2, 2, -2] }}
