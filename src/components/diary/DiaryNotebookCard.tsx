...
const DiaryNotebookCard: React.FC<DiaryNotebookCardProps> = ({ theme, diary, onClick }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "relative w-full p-4 rounded-2xl border border-border/50",
    ...
    >
      {/* Emoji badge */}
      <div className="absolute -top-2 -right-2 w-10 h-10 rounded-full bg-card shadow-soft flex items-center justify-center text-xl">
        {themeConfig.emoji}
      </div>
      
      <div className="pr-8">
        <h3 className={cn("font-semibold text-foreground mb-1", themeConfig.color)}>
          {themeConfig.label}
        </h3>
