...
- Se l'utente esprime pensieri di autolesionismo, rispondi con empatia e suggerisci di parlare con un professionista (Telefono Amico: 02 2327 2327).`;
    ...
    // Extract key fact and update memory (async, non-blocking)
    ...
    fetch('https://ai.gateway.lovable.dev/v1/chat/completions', {
      ...
      if (memResponse.ok) {
        ...
        if (memoryFact && memoryFact !== 'NESSUN_AGGIORNAMENTO' && memoryFact.length > 5) {
          ...
          // Use service role to update memory
          ...
          const updatedMemory = [...currentMemory, factWithContext].slice(-20); // Keep last 20 facts
          ...
          console.log('Memory updated with:', factWithContext);
        }
      }
